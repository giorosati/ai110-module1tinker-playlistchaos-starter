...
def normalize_field(value: str, lowercase: bool = True) -> str:
    """Normalize a string field for comparisons."""
...
def normalize_song(raw: Song) -> Song:
    ...
    title = normalize_field(str(raw.get("title", "")), lowercase=False)
    artist = normalize_field(str(raw.get("artist", "")))
    genre = normalize_field(str(raw.get("genre", "")))
